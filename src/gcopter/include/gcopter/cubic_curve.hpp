...
public:
    ...
    inline Eigen::Matrix2Xd getPositions() const //每一段的初始位置+最后一个位置
    {
        const int N = getPieceNum();
    ...
    inline Eigen::Vector2d getPos(double t) const //片段上的位置
    {
        const int pieceIdx = locatePieceIdx(t);
        return pieces[pieceIdx].getPos(t);
    }

    inline Eigen::Vector2d getJuncPos(const int juncIdx) const //得到连接点位置
    {
