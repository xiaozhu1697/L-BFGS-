...
namespace cubic_spline
{
    ...
    /*
        对带状线性系统的求解是解决具有带状结构的系数矩阵的线性方程组 Ax = b 的过程。在这里，带状结构指的是矩阵中
        只有主对角线及其两侧的一些带有非零元素，其他地方都是零。这种结构的矩阵通常出现在某些科学和工程问题的离散化过程中。
        带状线性系统的求解有多种方法，其中一种常见的方法是使用 LU 分解。LU 分解将原始矩阵分解为一个下三角矩阵 L 和一个上三角矩阵 U 的乘积，
        即 A = LU。通过 LU 分解，可以将原始线性方程组拆分为两个步骤：先解下三角系统 Ly = b，然后解上三角系统 Ux = y。
    */
    ...
    public:
        ...
        inline void setInnerPoints(const Eigen::Ref<const Eigen::Matrix2Xd> &inPs)
        {
          //TODO
            ...
            coffs.row(0) = headP;
            coffs.row(1) = D.row(0);
            coffs.row(2) = 3 * (inPs.col(0)-headP).transpose() - 2 * D.row(0) - D.row(1);
            coffs.row(3) = 2 * (headP - inPs.col(0)).transpose() + D.row(0) + D.row(1);
            for(int i = 1; i < N-1; i++)
            {
                coffs.row(4 * i + 0) = inPs.col(i).transpose();
                coffs.row(4 * i + 1) = D.row(i);
                coffs.row(4 * i + 2) = 3 * (inPs.col(i) - inPs.col(i-1)).transpose() - 2*D.row(i) - D.row(i+1); 
                coffs.row(4 * i + 3) = 2 * (inPs.col(i -1)-inPs.col(i)).transpose() + D.row(i) + D.row(i+1);
            }
            coffs.row(4*(N - 1) +0) = inPs.col(N - 2).transpose();
            coffs.row(4*(N - 1) +1) = D.row(N-1);
            coffs.row(4*(N - 1) +2) = 3*(tailP - inPs.col(N-2)).transpose() - 2*D.row(N-1);
            coffs.row(4*(N - 1) +3) = 2 * (inPs.col(N-2)-tailP).transpose() + D.row(N-1);
        ...
        inline void getCurve(CubicCurve &curve) const
        {
          //TODO
            ...
            for(int i = 0; i < N; i++)
            {
                curve.emplace_back(1,coffs.block<4,2>(4*i,0).transpose().rowwise().reverse());
            }
           return;
        }
        ...
        inline void getStretchEnergy(double &energy) const
        {
           ...
           for(int i = 0; i < N; i++)
           {
                energy += 4 * coffs.row(4 * i + 2).squaredNorm() + 
                          12 * coffs.row(4*i+2).dot(coffs.row(4*i+3)) +
                          12 * coffs.row(4*i +3).squaredNorm();
           }
            return;
        }
        ...
        inline void getGrad(Eigen::Ref<Eigen::Matrix2Xd> gradByPoints) const
        {
            ...
            for (int i = 0; i < N; ++i)
            {
                Eigen::Vector2d c_i = coffs.row(4 * i + 2);
                Eigen::Vector2d d_i = coffs.row(4 * i + 3);
